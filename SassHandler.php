...
class SassHandler extends Component
{
     ...
     * @var string
     */
    public $cachePath = '@runtime/sass-cache';
     ...
     *     Yii::getAlias('application.files')
     * );
    ...
    protected function publishInside(
        ...
        $insidePublishedDirectoryRealPath = Yii::getAlias(
            $insidePublishedDirectory
        );
    ...
    protected function setImportPaths($paths)
    {
        ...
        foreach ($paths as $originalPath) {
            $preparedPath = Yii::getAlias($originalPath);
    ...
    protected function cacheSet($name, $value)
    {
        ...
        if (!file_put_contents($path, serialize($value), LOCK_EX)) {
            throw new Exception('Can not write to the cache file: ' . $path);
        }
